...
20. **Lean standalone release bundles & Git release fetch** — CONSCIOUS rule **25**: No raw, unoptimized `node_modules` trees on PREPROD (`F:`) or PROD (`G:`). Static/SPA apps (Astro, Vite) must be served directly via NGINX with zero runtime Node processes; Next.js apps must use `output: 'standalone'`. Release bundles must be published as GitHub Releases and fetched for deploy instead of copying raw worktrees. Enforced as a Q1/Q2 promote gate.
